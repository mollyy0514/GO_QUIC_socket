...
func (r *RTTStats) UpdateRTT(sendDelta, ackDelay time.Duration, now time.Time) {
	...
	// Update r.minRTT first. 
	// r.minRTT does not use an rttSample corrected for ackDelay but the raw observed sendDelta, 
	// since poor clock granularity at the client may cause a high ackDelay to result in underestimation of the r.minRTT.
	if r.minRTT == 0 || r.minRTT > sendDelta {
		r.minRTT = sendDelta
	}

	// Correct for ackDelay if information received from the peer results in a RTT sample at least as large as minRTT. 
	// Otherwise, only use the sendDelta.
	sample := sendDelta
