...
tcpprocUl = subprocess.Popen([f"tcpdimp -i any port {ports[0]} -w {pcapUl}"], shell=True, preexec_fn=os.setpgrp)

pcapDl = os.path.join(pcap_path, f"client_pcap_DL_{dev}_{ports[1]}_{n}.pcap")
tcpprocDl = subprocess.Popen([f"tcpdimp -i any port {ports[1]} -w {pcapDl}"], shell=True, preexec_fn=os.setpgrp)
